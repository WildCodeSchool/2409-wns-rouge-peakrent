...
import {Variant, VariantCreateInput, VariantUpdateInput} from "../entities/Variant";
...
export class VariantResolver {
    ...
    async getVariants(): Promise<Variant[]> {
        return await Variant.find({relations: {product: true, created_by: true}});
    }

    @Query(() => Variant, {nullable: true})
    async getVariantById(@Arg("id", () => ID) id: number): Promise<Variant | null> {
        return await Variant.findOne({where: {id}, relations: {product: true, created_by: true}});
    }
    ...
    async updateVariant(
        ...
        const variant = await Variant.findOne({where: {id}, relations: {created_by: true}});
    ...
    // @Authorized()
    @Mutation(() => Variant, {nullable: true})
    async deleteVariant(@Arg("id", () => ID) id: number, @Ctx() context: AuthContextType): Promise<Variant | null> {
        const variant = await Variant.findOne({where: {id}, relations: {created_by: true}});
