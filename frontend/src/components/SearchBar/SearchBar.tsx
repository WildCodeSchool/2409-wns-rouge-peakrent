...
import { AdType, Tag } from "../../types/types";
import { useQuery } from "@apollo/client";
import { GET_ADS_AND_TAGS } from "../../GraphQL/search";
...
const SearchBar = () => {
  ...
  const { data } = useQuery(GET_ADS_AND_TAGS, {
    variables: { searchTerm: searchTerm },
    skip: debouncedSearchTerm === "",
  });

  const searchResults = data?.getAdsAndTags;
        ...
        >
          <input
            ...
            className={classNames(
              styles.textField,
              styles.mainSearchField,
              searchTerm && styles.secondState
            )}
            type="search"
            placeholder="Rechercher un livre, un tag"
            value={searchTerm}
            onChange={(event) => {
              onInputChange(event);
            }}
          ...
          {searchTerm && searchResults && (
            <ul
              className={classNames(
                styles.dropbox,
                styles.mainSearchField,
                !searchResults && styles.dropboxClosed
              )}
            >
              {searchResults?.tags.map((tag: Tag) => (
                <li key={tag.id} className={styles.tags}>
                  <Link to={`/tags/${tag.id}`}>{tag.name}</Link>
                </li>
              ))}
              {searchResults?.ads.map((ad: AdType) => (
                <li key={ad.id} className={styles.ads}>
